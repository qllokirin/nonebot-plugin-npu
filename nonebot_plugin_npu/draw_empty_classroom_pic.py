...
@run_sync
def draw_empty_classroom_pic(
    folder_path, unit_list, building_all, empty_classroom_all_data
):
    ...
    for empty_classroom_data, building in zip(empty_classroom_all_data, building_all):
        if empty_classroom_data["code"] != 200:
            empty_classroom_result.append(
                {"success": False, "building": building, "pic_base64": ""}
            )
        else:
            empty_classroom_data_today = empty_classroom_data["data"]["countMap"][
                str(datetime.today().isoweekday())
            ]
            ...
            # 画各个教室文字 计算列间距
            column_spacing = (canvas_width - left_content_gap) / len(
                empty_classroom_data_today
            )
            ...
            for i, every_empty_classroom in enumerate(empty_classroom_data_today):
                if not every_empty_classroom["isIdle"]:
                    unit_list_not_idle = [
                        ...
                        if unit not in every_empty_classroom["unitList"]
                    ]
            ...
            empty_classroom_result.append(
                {"success": True, "building": building, "pic_base64": img_base64}
            )
    msg_all = []
